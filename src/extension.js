...
let roomBar, musicBar, onlineBar, userBar, playerPanel, msgBar, mainPanel;
let msgList = [];
exports.activate = function (context) {
    ...
    let bbbug = {
        ...
        createMainPanel() {
            let that = this;
            if (that.data.isMainPanelShow) {
                that.sendDataToWebview();
                console.log("xxxxxxxxxxxxxx")
            } else {
                mainPanel = vscode.window.createWebviewPanel(
                    'hamm.chat.view', // viewType
                    "BBBUG音乐聊天室", // 视图标题
                ...
                mainPanel.onDidDispose(function () {
                    that.data.isMainPanelShow = false;
                });
                mainPanel.webview.onDidReceiveMessage(function (message) {
                    switch (message.evt) {
                        case 'init':
                            that.sendDataToWebview();
                            break;
                ...
                fs.readFile(context.extensionPath + '/src/template.html', 'utf8', function (err, data) {
                    mainPanel.webview.html = data
                    that.data.isMainPanelShow = true
                });
            }
        },
        initAudioPanel() {
            ...
            try {
                ...
                switch (obj.type) {
                    ...
                    case 'playSong':
                        ...
                        let song = "$(clock)" + obj.song.name + "(" + obj.song.singer + ")";
                        if (song.length > 30) {
        ...
        sendDataToWebview() {
            ...
            mainPanel.webview.postMessage({
                evt: "reload",
                data: {
                    access_token: that.data.postBase.access_token,
                    roomInfo: that.data.roomInfo,
    ...
    if (_access_token) {
        ...
        bbbug.request({
            ...
            success(res) {
                ...
                userBar.text = "帐号:" + decodeURIComponent(res.data.user_name);
